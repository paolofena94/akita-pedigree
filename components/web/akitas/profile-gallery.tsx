...
export function ProfileGallery3() {
    const [api, setApi] = useState<CarouselApi>();
    const [activeIndex, setActiveIndex] = useState(0);
    ...
    return (
        <>
            <div className="relative z-20 w-full aspect-square overflow-hidden group shadow-sm shrink-0">
                ...
                {/* TASTO ESPANDI (In alto a destra, appare in hover) */}
                <button
                    onClick={() => setIsLightboxOpen(true)}
                    className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full bg-white/70 backdrop-blur-md text-slate-800 shadow-inner opacity-0 group-hover:opacity-100 transition-all duration-300 hover:bg-white/90 active:scale-95 z-30"
                    aria-label="Espandi immagine"
                ...
                {/* Controlli di navigazione (Solo se c'è più di 1 foto) */}
                {images.length > 1 && (
                    <>
                        <div className="absolute inset-x-0 bottom-0 h-20 bg-linear-to-t from-slate-950/25 to-transparent pointer-events-none z-20" />
            ...
            {/* =========================================
                MODALITÀ SCHERMO INTERO (LIGHTBOX)
            ========================================== */}
            {isLightboxOpen && (
                <div className="fixed inset-0 z-100 flex items-center justify-center bg-slate-950/85 backdrop-blur-sm animate-in fade-in duration-200">
                    ...
                    <button
                        onClick={() => setIsLightboxOpen(false)}
                        className="absolute top-6 right-6 p-2 text-white/50 hover:text-white bg-white/5 hover:bg-white/10 rounded-full transition-all duration-200 z-50"
                        aria-label="Chiudi"
                    >
                        <X className="w-6 h-6" />
                    </button>
                    ...
                    <div className="relative w-full h-full max-w-6xl max-h-[85vh] mx-auto p-4 md:p-12 flex items-center justify-center">
                        <Image
                            ...
                            alt={`Foto a tutto schermo ${activeIndex + 1}`}
                            fill
                            className="object-contain" // object-contain assicura che la foto non venga MAI tagliata qui
                            quality={100} // Richiede la massima qualità per la visualizzazione grande
                            priority
                        />
                    </div>

                    {/* Navigazione anche nella Lightbox */}
                    {images.length > 1 && (
                        <>
                            {activeIndex > 0 && (
                                <button
                                    onClick={() => api?.scrollPrev()}
                                    className="absolute left-4 md:left-8 top-1/2 -translate-y-1/2 p-3 rounded-full bg-white/5 hover:bg-white/10 text-white backdrop-blur-md transition-all duration-200 z-50"
                                >
                                    <ChevronLeft className="w-8 h-8" />
                                </button>
                            )}
                            ...
                            {activeIndex < images.length - 1 && (
                                <button
                                    onClick={() => api?.scrollNext()}
                                    className="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 p-3 rounded-full bg-white/5 hover:bg-white/10 text-white backdrop-blur-md transition-all duration-200 z-50"
                                >
                                    <ChevronRight className="w-8 h-8" />
                                </button>
                            )}
...
export function ProfileGallery1() {
    const [api, setApi] = useState<CarouselApi>();
    const [activeIndex, setActiveIndex] = useState(0);

    // Nuovo stato per la modalità a schermo intero (Lightbox)
    const [isLightboxOpen, setIsLightboxOpen] = useState(false);
    ...
    return (
        <>
            <div className="relative z-20 w-full aspect-square rounded-4xl overflow-hidden group shadow-sm shrink-0 bg-slate-100 border border-slate-100">
